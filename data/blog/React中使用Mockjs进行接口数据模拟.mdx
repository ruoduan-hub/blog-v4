...
draft: false
summary: >-
  React 中使用 Mockjs 进行接口数据模拟 引子 每个改进都是为了解决问题。 现在我在开发中碰到了问题，我先描述下问题：
  我们现在做前后端完全分离的应用，前端写前端的，后端写后端的，他们通过 API 接口连接。 前端同学心理路程："后端同学接口写的好慢，我都没法调试了。"
  是不是有这个问题呢？一般我们怎么解决？
authors:
  - default
---
...
> 我们现在做前后端完全分离的应用，前端写前端的，后端写后端的，它们通过 API 接口连接。
...
>
> 但是想象一下，我们要模拟一个文章列表，就要手动写几十列。oh~no!
> 并且，后端接口一般都不带 .json，到时候对接，是不是还得改代码？

> 好了，下面介绍今天的主角 Mock.js。
> 它会做一件事情：拦截 AJAX 请求，返回需要的数据！
> 我们写 AJAX 请求的时候正常写，Mock.js 会自动拦截。
...
![Mockjs](/static/blog/React中使用Mockjs进行接口数据模拟/morkGif.gif)
