...
draft: false
summary: >-
  缘起 在每次 master bulid 后都要去阿里云 后台手动刷新｜预热，此文章旨在优化发包繁琐步骤 流程 实现 1. pip install
  aliyunpythonsdkcdn ps：这个SDK 和 下面的 代码 是基于 python2 推荐2.7.16 Mac 的话 默认版本就是
  python2 1. Refr
authors:
  - default
---
...
> 在每次 master 分支 build 后都要去阿里云后台手动刷新/预热，此文章旨在优化发包繁琐步骤。
...
> ps：这个 SDK 和下面的代码是基于 Python 2 的，推荐 2.7.16。
>
> Mac 的话默认版本就是 Python 2。
...
- 这只是个基本版，可以根据 SDK 参数来增加更多选项，参数如下：
