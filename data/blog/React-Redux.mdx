...
draft: false
summary: >-
  Redux and Reactredux 莞式教程（新手必看） 为了方便使用者使用，这里就 redux 和 reactredux 一并讲究使用了，会了
  reactredux，redux 也就没问题了，只不过稍微封装了一下 教程方式 1. 我们先讲理论，先明白 render、action、 store
  、connect 
authors:
  - default
---
...
![reduxflow](/static/blog/React-Redux/reduxflow.png)
...
> 这一章我们简单讲一下 react-redux，就开始我的莞式教程

- 首先通过 Provider 包裹根组件，让所有子组件都能访问到 store。
- 接着在需要接收或触发状态的子组件里，用 `connect()` 把当前组件和 store 连接起来。
- 通过 `mapStateToProps` 把数据（state）映射到组件的 props 里。
- 通过 `mapDispatchToProps` 把 action 打包成可以触发的函数，也传入 props。
- 最后，在组件内部调用 `this.props.xxx()` 就能触发状态变更，非常简单粗暴~
