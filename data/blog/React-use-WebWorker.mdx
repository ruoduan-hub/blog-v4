...
> Web Worker 老生常谈了 —— 它允许你在主线程之外创建额外的线程来执行任务，例如处理文件、埋点轮询等等。那么，如何在 React 中使用呢？
...
- `new URL(path, base)` 将路径和基础路径写入内存，供主线程引用。
...
至于 `worker-loader`，则是将 `new Worker(workPath)` 的步骤内置到 loader 处理流程中，并导出一个函数。外面直接使用该函数即可创建指定的 Worker：
...
> 还有一种方式：将 `worker.js` 的主函数转化为 `blobUrl` 导出，供主线程引用。该方法的好处是可以动态创建 worker。
