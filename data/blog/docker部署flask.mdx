...
draft: false
summary: >-
  Docker 部署 Flask 最近在用 Monorepo = (Flask + React) + (Python + TypeScript)
  写一些自己东西，记于此，温故而知新 WSGI Server Python 项目不同于其他服务例如 javax.servlet.Servlet \ Go \
  PHP 等 ，Pyt
authors:
  - default
---
...
<img src="https://s2.loli.net/2022/08/05/ZJg4A6Ml39Fvu5z.png" alt="docker" data-zoom="50" />
...
Gunicorn 给 WSGI 组件提供服务器功能，但 Gunicorn 默认使用同步阻塞的网络模型（`-k sync`），对于大并发的访问可能表现不够好。配合 Gevent 来增加并发量，就可以组成一个 "高性能" 服务器。
...
正常启动的话，我们可以看到服务已经运行了，可以通过 http://localhost:5000/ 正常访问。
...
容器正常运行，我们现在要做的就是上传镜像，然后在服务器上拉取并启动容器。
...
<img src="https://s2.loli.net/2022/08/05/kZYfbNTSPHOonME.png" alt="image-20220805154622541" data-zoom="50" />



<img src="https://s2.loli.net/2022/08/05/NFqZxKmMsfWk4Gb.png" alt="image-20220805154727853" data-zoom="50" />
...
> 把操作系统的 80 端口映射到容器的 5000 端口，这时候再去开放安全组就可以正常访问了。
