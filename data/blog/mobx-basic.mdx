...
draft: false
summary: >-
  Mobxbasic(快速上手)莞式教程 之前写过一篇 redux 的教程 可以参考 本篇讲通过通俗易懂的方式让你快速上手 mobx
  的基本使用，虽然已经可以满足大部分开发中的需求但是详细 请参考 本篇配 https://github.com/Chad97/mobxbasic
  如果对想要快速入门的你有所帮助请给点个小小的
authors:
  - default
---
...
![mobx](/static/blog/mobx-basic/mobx1.png)
...
因为本篇通篇使用 ES7 中的修饰器语法 `@`，所以需要配置 babel，推荐使用。
当然你也可以使用 [decorators](https://cn.mobx.js.org/refguide/modifiers.html) 作为参考。
...
如果你用过 redux，那这里对你来说简直不要太简单，只不过 mobx 更加面向对象。
...
当然，如果你的组件嵌套太深，觉得上面两种方式太麻烦，也可以使用 `React Context` 来传递 `newState`，这里就不做展开了，[具体参考](https://react.docschina.org/docs/context.html)。

#### 运行机制

贴一张官方的图
![mobx-flow](/static/blog/mobx-basic/mobx-flow.png)
可以发现，对比 redux，mobx 的方式让人更容易理解和使用。
面向对象的方式用起来也不用写大量的模板代码，比较清爽。

redux 教程链接 https://blog.csdn.net/ruoduan-hub/article/details/89522403

最后，大家可以 clone 我的 demo 教程作为参考哦~
