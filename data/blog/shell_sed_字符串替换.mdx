...
categories:
  - Shell
draft: false
summary: "缘起 在构建 Build 的时候需要根据平台和环境写入配置文件, 需要写一个脚本来提供不同的模板,记于此 例如 \U0001F447\U0001F3FB 模板文件: 代码 可以使用 shell 的 sed 命令就行替换 在当前文件替换修改加 i \U0001F449\U0001F3FB sed i \"s/{{base}}//g\" $filename 但是在 MacOS中 不行 需使"
authors:
  - default
---
