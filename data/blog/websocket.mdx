...
draft: false
summary: >-
  岗位 web 前端 我的 csdn： 个人博客：Ruoduan.top 敲黑板 知识点: websocket： websocket 是 HTML5
  的一个新协议，它允许服务端向客户端传递信息，实现浏览器和客户端双工通信。websocket 弥补了 HTTP 不支持长连接的特点，那么在学习
  websocket 之前我们先
authors:
  - default
---
...
websocket 是 HTML5 的一个新协议，它允许服务端向客户端传递信息，实现浏览器和客户端双工通信。websocket 弥补了 HTTP 不支持长连接的特点。在学习 websocket 之前，我们先来了解一下 HTTP。

# HTTP

HTTP 是用于传输如 HTML 文件,图片文件，查询结果的应用层协议。它被设计于用于服务端和客户端之间的通信。在工作的时候，客户端打开一个连接以发出请求，然后等待服务端响应，服务端不能主动向客户端发送请求。HTTP 是无状态协议，意味着服务器不会在两个请求之间保留任何数据。那么这就带来了一个问题，比如说在一个电商网站中，把某个物品加入了购物车，换了一个页面后再添加一个物品，两次添加物品的请求之间没有联系，浏览器无法知道用户选择了哪些商品。解决方法是在 HTTP 头部中加入 cookie 信息，这样每次请求都能够共享相同的状态。
...
关于 HTTP 的内容不多讲了，推荐大家去看《HTTP 权威指南》，很小的一本书，讲得很清楚。

# HTTPS

HTTPS 是什么呢，它与 HTTP 的关系又是什么？在上一段讲过 HTTP 是我们平时浏览网页时使用的一种协议。HTTP 协议传输的数据都是没有加密的，也就是明文的，因此使用 HTTP 协议传输隐私信息非常的不安全。**为了让这些隐私数据能够加密传输，便设计了 SSL 协议对 HTTP 传输的数据进行加密，从而诞生了 HTTPS。**而后对 SSL 进行不断的升级，出现了 TLS。但是呢，名字用久了也就产生了感情，所以也一直延续着 SSL 是 HTTPS 的代名词的习惯。
下面这张图能够让你很清晰的认识到 HTTP 和 HTTPS 之间的关系这儿有一张图

![http](/static/blog/websocket/HTTP1.png)
...
讲了这么多终于讲到了 websocket。websocket 相对于 HTTP 协议来说，是一个持久化的协议。下面是一个典型的 websocket 握手
...
可以看到相对于 HTTP 的头部多了几个字段。其实我们可以说，websocket 借用了 HTTP 的握手，是 HTTP 的一个解决特定问题的补丁。

我们看看上面的头部相对 HTTP 头部都有哪些变化：
...
![websocket2](/static/blog/websocket/HTTP2.png))

可以看到 HTTP 获取数据的时候，需要不断地问服务端："有没有我要的数据啊？" 如果有数据就返回，没有就过一段时间再问一次。而 websocket 呢？它只建立一次连接，这个连接就不会断，服务端如果有数据的话，会自动推送给客户端。

还有一个问题，在 HTTP 中我们提到 HTTP 是无状态的，也就是它 "健忘"，上一次的请求和这次的请求没什么联系，我们需要引入 cookie 才能解决。而在 websocket 中，因为是一次长连接，就不用一次次携带 cookie 了，是不是方便很多？

下面的代码就是 websocket 在前端代码中的应用：
...
![heap](/static/blog/websocket/Heap1.png)
...
![heap2](/static/blog/websocket/Heap2.png)
