...
## 缘起
> 在使用 `npm` 包的过程中有一些特性不支持，可以通过修改源码来完成。
> 尽管已经可以通过 Issues | PR 来解决，但开源社区审核较慢，这时候就可以通过 `patch-package` 来快速完成。
...
大概意思就是说：大多数情况下在 npm 包安装和卸载的时候，`patch-package` 会通过 diff 去替换包内的源码。
但是 yarn 只会在安装的时候执行，卸载的时候不会去执行。使用 `postinstall-postinstall` 后可以确保卸载的时候也会执行。


## 生成源码
> 以 antd 为例子 🌰

我们首先去 fork antd 的代码拉到本地后，修改源码，跑过单元测试等等。
...
> `patch-package` 是锁定版本的，所以在 `package.json` 里最好把版本锁定到当前打了补丁的这个版本。
