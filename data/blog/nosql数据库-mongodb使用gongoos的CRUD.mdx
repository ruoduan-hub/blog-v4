...
draft: false
summary: >-
  简介 NoSQL，指的是非关系型的数据库。NoSQL 有时也称作 Not Only SQL 的缩写，是对不同于传统的关系型数据库的数据库管理系统的统称。
  NoSQL 用于超大规模数据的存储。（例如谷歌或 Facebook
  每天为他们的用户收集万亿比特的数据）。这些类型的数据存储不需要固定的模式，无需多余操作就可以横向扩
authors:
  - default
---
...
下载完成后点击安装：
![安装](/static/blog/nosql数据库-mongodb使用gongoos的CRUD/1.png)

完成安装顾名思义 next 就完了

我这里选择的自定义安装 看看有哪些东西

![安装](/static/blog/nosql数据库-mongodb使用gongoos的CRUD/2.png)

**next**

![安装](/static/blog/nosql数据库-mongodb使用gongoos的CRUD/3.png)
...
- 用 mongod -- vesrion
  ![安装](/static/blog/nosql数据库-mongodb使用gongoos的CRUD/4.png)

可以看到 并没有 显示版本号 ，因为我们还没有配置环境变量，下面来配置一下环境变量

### 配置环境变量

![安装](/static/blog/nosql数据库-mongodb使用gongoos的CRUD/5.png)

- 打开安装路径找到 bin 的路径复制

![安装](/static/blog/nosql数据库-mongodb使用gongoos的CRUD/6.png)

找到系统-环境变量
![安装](/static/blog/nosql数据库-mongodb使用gongoos的CRUD/7.png)

- 现在我们再来 mongod -version
  ![安装](/static/blog/nosql数据库-mongodb使用gongoos的CRUD/8.png)
...
**mongodb 默认使用 mongod 命令所在的盘符的磁盘根目录作为自己的数据存储目录也就是
目录盘符/data/db**
...
![安装](/static/blog/nosql数据库-mongodb使用gongoos的CRUD/连接数据库.png)
